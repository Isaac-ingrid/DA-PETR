...
model = dict(
    ...
    img_backbone=dict(
        ...
        frozen_stages=-1, norm_cfg=dict(type='BN2d', requires_grad=False),
        norm_eval=True, style='caffe', with_cp=True,
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            decoder=dict(
                type='PETRTransformerDecoder', return_intermediate=True, num_layers=6,
                transformerlayers=dict(
                    ...
                    feedforward_channels=2048, ffn_dropout=0.1, with_cp=True,
        ...
        bbox_coder=dict(
            ...
            max_num=900, voxel_size=voxel_size,
            num_classes=5,              # ← 统一为 5
        ),
...
# dataset_type = 'CustomLyftDataset'     # ← 训练/验证/测试都用你自定义的
dataset_type = 'DACustomNuScenesDataset'
data_root = '/data/dataset/lyft/'
...
data = dict(
    ...
    train=dict(
        ...
        classes=class_names,           # ← 传入 5 类，Dataset 会据此重映射 GT
    ...
    val=dict(
        ...
        test_mode=True,
        box_type_3d='LiDAR',
        # map_nusc_to_lyft=False  # 评测 Lyft 5 类，不做 10→5 映射
    ),
    test=dict(
        type=dataset_type,
        data_root=data_root,
